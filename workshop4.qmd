...
The methods below show you how to do this. We first start by assuming that the grouping variables do not interact (ie: combinations of two factors don't cause different outcomes to each factor alone), in what is called an **additive model (or multiplicative) model**.
...
But the three grouping variables may have interacting effects, and not be independent. So then we then examine a non additive model. You'll see more clearly what this means as you do the analysis.
...
**Interpretation:** Where the p-values `Pr(>F)` are < 0.05, we have a significant effect of this factor on the pNP level (and therefore osteogenesis). The multiplicative tests all single factors, all pairwise factors (eg: `day:clone`) and th interaction of all three factors(`day:clone:induced`).
