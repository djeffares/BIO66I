...
To create the standard curve you have the absorbance readings for 8 concentrations of pNP in triplicate. To measure osteogenesis in the two clonal lines, samples of the cells were grown in an inductive media for 8 days. The cells were then lysed at different time points to measure the activity of alkaline phosphatase and explore how it varies over time and between the two clonal lines. The clonal lines were analysed at Day 0, Day 4 and Day 8. At each time point you have data for both induced cells and non-induced cells (negative control), with the exception of Day 0 as the differentiation was induced at Day 1. There are three repeated measures for the data at each time point.
...
First, read in some ata from an Excel file. You can obtain [an example file from here](https://docs.google.com/spreadsheets/d/1xcRoGBI8CoAu343Et2aiVSfoZ7FGBtBy/edit?usp=sharing&ouid=113672614936656202737&rtpof=true&sd=true). Click to download, and then save the file in your **raw-data** directory. If you don't have a **raw-data** directory, make one!
...
Now we have the linear model, if we have some some pNP absorbance values, we can determine of the pNP product concentrations. We have put some mock pNP values in the excel file `example_alkaline_phosphatase_activity_assay-2024-03-21.xlsx`, in sheet two.
...
:::callout-note
# Why normalise?
During the osteogenic differentiation of the clonal lines, the cells will proliferate at different rates. In basal conditions clone A normally grows slightly slower than clone B, and hMSCs also grow quicker in osteoinductive media, therefore we need to normalise the results of the experiment relative to cell number. Without normalisation, if we observe an increase in pNP concentration (and therefore ALP activity) in some of the conditions we cannot infer this is because the enzyme is more highly expressed. It could simply be due to the presence of a greater number of cells that are expressing the same amount of ALP as another condition, but there are just more cells present. 
:::

So we need to normalise these differences in growth rate. We can do this by dividing the predicted pNP concentration by the DNA concentration, because we know that cells **don't** gain or lose large amounts of DNA very often!

To achieve this, we can use the `mutate` function to create a new column in the data frame, `mock`. We will call this new column `normalised.pNP.concs`. We will divide the `predicted.pNP.concs` column by the `DNA.conc` column.
...
Noe let's make a plot (with the **normalised** data). This will account for differential growth rates of the clones.
