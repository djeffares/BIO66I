...
-   In our case we use absorbanceance (***x*** **values**) to predict alkaline phosphatase activity (***y*** **values**)
...
In practical 4 you have examined their capacity to undergo *osteogenesis* (differentiate into osteoblasts - bone cells) when they are given an inductive medium. To recap:
...
### Part 1: pNP absorbanceance values

In this section, we show how to use a linear model to predict pNP concentration from absorbanceance values using some **example data**. In your report, use **your own data** from practical 4.
...
We can use the `lm` function to make a linear model. This is a simple model that assumes that the absorbanceance is linearly related to the pNP concentration. We can use this model to predict the pNP concentration of unknown samples.
...
Now we have the linear model, if we have some pNP absorbanceance values, we can determine the pNP product concentrations. 
...
We named our mean absorbanceance `absorbance` because we specified the `absorbance` variable in the code `linear_model <- lm(pNP.conc ~ absorbance , data = pnp.calibration.pivot)` above. So our linear model is 'looking for' an absorbance variable to use.
...
To achieve this, we can use the `mutate` function to create a new column in the data frame, `pnp.experimental`. We will call this new column `normalised.pNP.concs`. We will divide the `predicted.pNP.concs` column by the `DNA.conc` column.
...
In practical 4, you collected absorbanceance readings for concentrations of pNP in triplicate, to provide an indication of the two clones to undergo osteogenesis. pNP concentrations were measured in day 0,4 and 8, and at each time point you have data for both induced cells and non-induced cells (negative control), with the exception of day 0 as the differentiation was induced at day 1.
...
Now we use `pivot_longer` to reformat the data, so we can use all the repeats. The `cols=!c(day,clone,differentiated)` part indicates that the day,clone,differentiated columns are not to be lengthened into the 'absorbance' column. So it is `onlyabsorptionrep1`,`absorptionrep2` and `absorptionrep3` that are put into the 'absorbance' column.
...
If we look at the absorbanceance by day, we can see that absorbanceance goes up each day. Here we are ignoring the media (`induced`).
...
To find out if there is evidence that the absorbanceance value is affected by day, clone or media using an ANOVA, we can use the R function `aov()`.
