...
If we want to view how far many different cells have moved, we will need to adjust their point locations, so the **median** of their `x.position`s and `y.position`s are zero. Effectively setting all their start positions to x=0,y=0 on a grid.
