...
#cells <-read_tsv(url("https://djeffares.github.io/BIO66I/A2-FFT.csv"))
...
![**Figure 1. Tracking ID and Lineage ID. ** The **tracking ID (TID)** is a unique ID that the microscope assigned to each object (cell), as it tracks it through time. The **lineage ID (LID)** is a number assigned to the ‘family’ of cells that derived by cell division during the the experiment. When a cell divides, each daughter cell is given a new tracking ID, but they keep their lineage ID, because they are still part of the same cell lineage 'genealogy'.
](../images/tracking-id-cell-id.png){fig-align="centre" width=50% width=50%}
